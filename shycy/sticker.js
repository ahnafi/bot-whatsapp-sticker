...
export default {
    ...
    execute: async({ shicy, m, quoted, config }) => {
        ...
        if (/image|video|sticker/.test(quoted.mime)) {
            ...
            shicy.sendMessage(m.from, media, { asSticker: true, quoted: m, ...exif })
        } else if (m.mentions.length != 0) {
            let url
            for (let a = 0; a < (m.mentions.length < 4 ? m.mentions.length : 4); a++) {
                url = await shicy.getProfilePicUrl(m.mentions[a]).then(_ => _).catch(_ => 'https://lh3.googleusercontent.com/proxy/esjjzRYoXlhgNYXqU8Gf_3lu6V-eONTnymkLzdwQ6F6z0MWAqIwIpqgq_lk4caRIZF_0Uqb5U8NWNrJcaeTuCjp7xZlpL48JDx-qzAXSTh00AVVqBoT7MJ0259pik9mnQ1LldFLfHZUGDGY=w1200-h630-p-k-no-nu') 
            }
            shicy.sendMessage(m.from, url, { asSticker: true, quoted: m, packName: config.Exif.packName, packPublish: config.Exif.packPublish })
        } else if (/(https?:\/\/.*\.(?:png|jpg|jpeg|webp|mov|mp4|webm))/i.test(m.text)) {
            shicy.sendMessage(m.from, Func.isUrl(m.text)[0], { quoted: m, asSticker: true, packName: config.Exif.packName, packPublish: config.Exif.packPublish })
        } else {
